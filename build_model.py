...
def _intro_html(dpi):
    ...
    return (
        ...
        f'<p>{j_plain}, the plain unpolarized emissivity, is fixed at 1 across {j_p}\'s own '
        f'lower/upper bound (0 outside it) -- {j_p} above is taken to mean the local complex '
        f'polarization <i>degree and phase together</i>, so any magnitude dependence typed into '
        f'it (a spatial envelope, a bare {nu}/{lam} factor) reshapes {equation1}\'s result '
        f'directly rather than being divided back out; it\'s only ever clipped to magnitude 1 '
        f'wherever it would otherwise exceed 100% local polarization. {equation2} is then just '
        f'that region\'s own area, normalizing {equation1}\'s result so it depends only on the '
        f'bounds set below, not some arbitrary wider range {j_p} happens to be evaluated over. '
        f'(The one exception is an SSA/Thermal spectral shape, where {j_plain} is instead '
        f'derived from {j_p}\'s own shape -- see the Spectral model note below.)</p>'
