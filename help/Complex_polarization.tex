...
More generally, consider a source whose polarized emissivity at position $\vec r$ is $j_p(\lambda,\vec r) = p\,j(\lambda,\vec r)\,e^{2i\chi(\vec r)}$, i.e., a fraction $p$ of the total emissivity $j(\lambda,\vec r)$, emitted with its own local EVPA $\chi(\vec r)$. If radiation from each position $\vec r$ is then Faraday-rotated by its own Faraday depth $\phi(\vec r)$ on the way to the observer, the complex polarization is obtained by integrating along the entire emitting volume $V$ of the source:
\begin{equation}\label{eq:emissivity}
    \displaystyle P(\lambda) = \frac{\int j_p(\lambda,\vec r)\,e^{2i\phi(\vec r)\lambda^2}\,dV}{\int j(\lambda,\vec r)\,dV}
\end{equation}
Every polarization model below (and any custom model built in the "Custom models" tab, see the Custom models help page) amounts to a particular choice of $j_p(\lambda,\vec r)$ and $\phi(\vec r)$ plugged into Equation \ref{eq:emissivity}.

The screen is said to be external if it is spatially separated from the emitting region, meaning that the emission and rotation integrals in Equation \ref{eq:emissivity} can be solved separately. In this simplest configuration, if the screen is also uniform ($n_e$ and $B_\parallel$ do not depend on position), all the radiation that traverses it will have its EVPAs rotated by the same amount, without affecting $p$; a simple rotation of the EVPAs with a magnitude proportional to a quantity called the Rotation Measure (RM):
\begin{equation}\label{eq:RM}
    \text{RM} = \frac{d\chi}{d(\lambda^2)} \quad \textnormal{rad m}^{-2}
\end{equation}

For this specific and very simple situation, Equation \ref{eq:emissivity} reduces to $P(\lambda) = p\,e^{2i(\chi_0 + \phi\lambda^2)}$, where $\chi_0$ is the original (unrotated) EVPA, and the observed RM will be identical to the Faraday depth:
