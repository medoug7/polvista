...
Each channel's weight $w_i$ combines its own noise, as an inverse-variance weight $1/\sigma_i^2$, with a Briggs (robust) re-weighting across $\lambda^2$ that downweights crowded regions of the band and upweights sparser ones -- the same robust weighting radio-interferometric imaging applies across the $(u,v)$ plane, trading a little resolution for suppressed sidelobes when the coverage is uneven. The noise itself also propagates through Equation \ref{eq:dirty} into an expected noise level $\sigma_{\rm FDF}$ in the Faraday spectrum, used both as the RM-CLEAN stopping threshold below and, drawn on the plot, as a reference for how much of $F_{\rm dirty}(\phi)$ is signal rather than noise.
\\
